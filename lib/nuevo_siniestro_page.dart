...
class _NuevoSiniestroPageState extends State<NuevoSiniestroPage> {
  ...
  Future<void> _cargarClientes() async {
    ...
    setState(() {
      clientes = resultados;
      clienteSeleccionado = ClienteGlobal.seleccionado ?? (clientes.isNotEmpty ? clientes.first : null);
  ...
  Future<void> _guardarSiniestro() async {
    ...
    try {
      ...
      final estadoInicial = tipoObjeto == "mascota" ? "sin_novedad" : "en_uso";
      ...
      // Actualiza el estado del QR correspondiente
      final users = db.collection("users");
      await users.updateMany(
        mongo.where.eq("objetoId", objetoSeleccionado!["_id"]),
        mongo.modify.set("status", estadoInicial),
      );
      ...
      setState(() => objetoSeleccionado = objetos.isNotEmpty ? objetos.first : null);
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _fichaObjeto() {
    ...
    if (tipo == "carro") {
      ...
      detalles.add(Text("Estado inicial: Sin Novedad"));
    }
      ...
      body: Center(
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: Colors.black26, blurRadius: 18, offset: Offset(6, 10)),
            ],
          ),
                          ...
                          DropdownButtonFormField<mongo.ObjectId>(
                            ...
                            items: clientes.map((cliente) {
                              return DropdownMenuItem<mongo.ObjectId>(
                                ...
                                child: Text("${cliente["nombre"]} (${CryptoUtils.decryptText(cliente["cedula"] ?? '')})"),
                              );
                            }).toList(),
